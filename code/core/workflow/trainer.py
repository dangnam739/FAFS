...
def train_net(net,
    ...
    use_fft = cfg.DATASET.USE_FFT

    # ent_loss weight
    entW = cfg.DATASET.TARGET.ENT_LOSS_WEIGHT

    # train dataset - source
    if use_fft and cfg.DATASET.TARGET.ANNS != '':
        t_anns = cfg.DATASET.TARGET.ANNS
        t_image_dir = cfg.DATASET.TARGET.IMAGEDIR
        fft = [t_anns, t_image_dir]
        beta = cfg.DATASET.FFT_BETA
        fft_mode = cfg.DATASET.FFT_MODE

        train = DATASET[cfg.DATASET.TYPE](anns, image_dir, use_aug=use_aug, fft=fft, beta=beta, fft_mode=fft_mode)
    else:
    ...
    while iter_cnt < cfg.TRAIN.EPOCHES * len(train_data):
        ...
        if cfg.MODEL.TYPE == "UDA_Segmentor": #warmup
            # pdb.set_trace()
            loss_dict = net(
                source=images,
                target=t_images,
            ...
            if entW > 0: 
                loss_dict_tgt = net(t_images)
                loss_all = loss_dict_src["loss_ce"] + entW*loss_dict_tgt["loss_ent"]  # loss of seg on src, and ent on s and t
            else:
                loss_all = loss_dict_src["loss_ce"]
                
            loss_dict.update({"loss_all": loss_all})
