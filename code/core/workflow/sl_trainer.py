...
from ..datasets.loader.gtav_dataset import GTAVDataset, MsGTAVDataset
from ..datasets.loader.cityscapes_dataset import CityscapesDataset, MsCityscapesDataset
...
def self_train_net(net, net_pseudo, cfg, gpu):
    ...
    dist.init_process_group(
        backend='nccl',
        init_method='tcp://127.0.0.1:6788',
    ...
    update_pseudo_label = cfg.DATASET.TARGET.PSEUDO_PREV_UPDATE
    ...
    with torch.no_grad():
        ...
        if update_pseudo_label:
            # print(pseudo_save_dir)
            pseudo_save_dir_lst = list(pseudo_save_dir)
            pseudo_save_dir_lst[-14] = str(int(pseudo_save_dir_lst[-14]) - 1)
            prev_pseudo_save_dir = ''.join(pseudo_save_dir_lst)
        else:
            prev_pseudo_save_dir = None

        if len(os.listdir(pseudo_save_dir)) < 2975:
            generate_pseudo_labels(cfg, net_pseudo, t_train_data_sl, pseudo_save_dir, gpu, prev_pseudo_save_dir=prev_pseudo_save_dir, color=True)
    ...
    while iter_cnt < cfg.TRAIN.EPOCHES * len(train_data):
        ...
        try:
            ...
            s = next(source_iter)
            # epoch += 1
        if iter_cnt>0 and iter_cnt%len(train_data)==0:
            epoch += 1
...
def generate_pseudo_labels(cfg, net_pseudo, t_train_data_sl, pseudo_save_dir, gpu, prev_pseudo_save_dir=None, color=False):
    # gen pseudo-label
    n_class = cfg.MODEL.PREDICTOR.NUM_CLASSES
    ...
    if cfg.DATASET.TARGET.PSEUDO_PL == "IAST":
        ...
        for i, b in enumerate(t_train_data_sl):
            ...
            for _i, name in enumerate(names):
                ...
                if prev_pseudo_save_dir is not None:
                    prev_pseudo_label_path = os.path.join(prev_pseudo_save_dir, pseudo_label_name)
                    prev_pseudo_label = Image.open(prev_pseudo_label_path)
                    prev_pseudo_label_np = np.array(prev_pseudo_label)

                    # update label from previous self-training
                    ignore_idx = np.argwhere(ignore_index==True)
                    for id in ignore_idx:
                        i, j = id
                        label[i][j] = prev_pseudo_label_np[i][j]
                ...
                if color:
                    pseudo_color_save_dir = os.path.join(pseudo_save_dir, "color")
                    if not os.path.exists(pseudo_color_save_dir):
                        try:
                            os.makedirs(pseudo_color_save_dir)
                        except Exception:
                            pass

                    pseudo_color_label_path = os.path.join(pseudo_color_save_dir, pseudo_color_label_name)
...
def colorize_mask(mask):
    ...
    mask = np.where(mask == 255, 19, mask)
