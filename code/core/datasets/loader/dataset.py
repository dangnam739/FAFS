...
from ..aug.fft import convert_tgt_style_yuv_mst, convert_tgt_style
...
class BaseDataset(Dataset):
    ...
    '''
    def __init__(self, anns, image_dir, scale=1, resize_size=None, center_crop=1, use_aug=False, num_classes=19, pseudo_dir=None, fft=None, beta=None, fft_mode=None):
        ...
        if self.fft:
            self.tgt_img_list, _ = read_anns(self.fft[0], self.fft[1])
        else:
            self.tgt_img_list = None
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if "CycleGan" in im_path:
                label = label.resize(im.size, Image.BICUBIC)

            # apply fast fourier transform
            if self.fft:
                im_tgt_path = random.choice(self.tgt_img_list)
                im = convert_tgt_style(im, im_tgt_path, im_path, beta=self.beta, mode=self.fft_mode)
